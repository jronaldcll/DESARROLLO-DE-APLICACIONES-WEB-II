...
export class TokenService implements OnDestroy {
  ...
  private _roleArray: { name: string; priority: number }[] = [];

  private _permissionArray: string[] = [];

  private get token(): BaseToken | undefined {
    if (!this._token) {
      const storedToken = this.store.get(this.key) as Token;
      if (storedToken) {
  ...
  public get roleArray(): { name: string; priority: number }[] {
    return this._roleArray;
  }
  public set roleArray(value: { name: string; priority: number }[]) {
